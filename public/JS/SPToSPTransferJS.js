...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Modified showAgentPINModal function
  function showAgentPINModal(recipient, amount, charge, commission, total) {
    // Create modal as a centered popup instead of full page
    ...
    <div class="verification-modal">
      ...
      <div class="form-header-gradient">
        <div class="header-content">
          <button type="button" class="back-btn verification-close-btn">
            <i class="fas fa-arrow-left"></i>
          </button>
          <div class="header-title">
            <h3>বিকাশ টু বিকাশ</h3>
          </div>
          <div class="bkash-logo">
            <img src="/public/images/bkashlogo.png" alt="বিকাশ লোগো" style="height: 30px;">
          </div>
        </div>
      </div>
      
      <div class="verification-modal-body">
        <!-- Transaction info card in style of image -->
        <div class="transaction-summary-card">
          <div class="summary-header">লেনদেনের বিবরণ</div>
          <div class="summary-row">
            <span class="summary-label">
              <i class="fas fa-exchange-alt"></i> ট্রান্সফার পরিমাণ
            </span>
            <span class="summary-value">৳${formatNumber(amount)}</span>
          </div>
          <div class="summary-row">
            <span class="summary-label">
              <i class="fas fa-tag"></i> সার্ভিস চার্জ
            </span>
            <span class="summary-value">৳${formatNumber(charge)}</span>
          </div>
          <div class="summary-row">
            <span class="summary-label">
              <i class="fas fa-star"></i> এজেন্ট কমিশন
            </span>
            <span class="summary-value">৳${formatNumber(commission)}</span>
          </div>
          <div class="summary-row total">
            <span class="summary-label">
              <i class="fas fa-wallet"></i> মোট খরচ
            </span>
            <span class="summary-value total-value">৳${formatNumber(
              total
            )}</span>
          </div>
        </div>
        
        <!-- PIN entry note section -->
        <div class="agent-verification-note">
          <i class="fas fa-info-circle verify-icon"></i>
          <div>এই সার্ভিসের মাধ্যমে আপনি আপনার এজেন্ট অ্যাকাউন্ট থেকে যেকোনো বিকাশ গ্রাহকের অ্যাকাউন্টে টাকা পাঠাতে পারবেন।</div>
        </div>
        
        <!-- PIN Entry with 5 boxes for physical keyboard input -->
        ...
        <!-- Action button -->
        <button type="button" class="action-btn transfer-pin-btn pulse-btn" id="verify-pin-btn">
  ...
  `;
    ...
    .verification-modal .form-header-gradient {
    ...
    .verification-modal .header-title h3 {
      font-size: 20px;
      font-weight: 600;
      color: white;
      margin: 0;
    ...
    .pin-box:focus {
      border-color: #1a8ae3;
      box-shadow: 0 0 0 2px rgba(26, 138, 227, 0.2);
    }
  ...
  `;
    ...
    // Select elements
    const pinBoxes = Array.from(modalOverlay.querySelectorAll(".pin-box"));
    const verifyPinBtn = modalOverlay.querySelector("#verify-pin-btn");
      ...
      box.addEventListener("input", () => {
        if (box.value.length === 1) {
          if (index < pinBoxes.length - 1) {
            ...
            if (allFilled) {
              box.blur(); // Remove focus from the last box
            }
          }
        }
      });
    ...
    const successHTML = `
      <div class="success-modal">
        <div class="success-content">
          ...
          <h2 class="success-title">ট্রান্সফার সফল হয়েছে!</h2>
          <p class="success-message">${recipient} নম্বরে ৳${formatNumber(
      amount
    )} টাকা সফলভাবে ট্রান্সফার করা হয়েছে</p>
          ...
          <div class="txn-details">
            ...
            <div class="txn-row">
              <span class="txn-label">সার্ভিস চার্জ</span>
              <span class="txn-value">৳${formatNumber(charge)}</span>
            </div>
            <div class="txn-row">
              <span class="txn-label">এজেন্ট কমিশন</span>
              <span class="txn-value">৳${formatNumber(commission)}</span>
            </div>
            <div class="txn-row total">
              <span class="txn-label">মোট খরচ</span>
              <span class="txn-value">৳${formatNumber(total)}</span>
            </div>
          </div>
          
          <div class="success-actions">
            <button class="success-btn new-transaction">নতুন ট্রান্সফার</button>
